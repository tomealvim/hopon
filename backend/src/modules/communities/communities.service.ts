...
export class CommunitiesService {
  ...
  async join(userId: string, inviteCode: string) {
    ...
    const requesterName =
      requester?.profile?.name ?? requester?.email ?? 'Alguém';
    ...
    if (existing) {
      if (existing.status === 'APPROVED')
        throw new ConflictException('Já és membro desta comunidade');
      if (existing.status === 'PENDING')
        throw new ConflictException('O teu pedido já está pendente');
      ...
      await this.prisma.communityMember.update({
        ...
        data: {
          status: newStatus,
          joinedAt: newStatus === 'APPROVED' ? new Date() : null,
        },
      });
  ...
  async getMembers(requesterId: string, communityId: string) {
    ...
    if (!membership || membership.status !== 'APPROVED')
      throw new ForbiddenException('Não tens acesso a esta comunidade');
  ...
  async removeMember(ownerId: string, communityId: string, userId: string) {
    const community = await this.assertOwner(ownerId, communityId);
    if (ownerId === userId)
      throw new ForbiddenException('O owner não pode sair da comunidade');
  ...
  async leave(userId: string, communityId: string) {
    ...
    if (member.role === 'OWNER')
      throw new ForbiddenException(
        'O owner não pode sair da comunidade. Transfere a liderança primeiro.',
      );
  ...
  async getSharedCommunityMap(
    ...
    const communityById = new Map(
      myMemberships.map((m) => [
        m.communityId,
        { id: m.community.id, name: m.community.name },
      ]),
    );
  ...
  private async assertOwner(userId: string, communityId: string) {
    ...
    if (community.ownerId !== userId)
      throw new ForbiddenException('Só o owner pode fazer esta ação');
    return community;
  }
}

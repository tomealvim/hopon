...
export class RideRequestsService {
  ...
  async close(passengerId: string, id: string) {
    const req = await this.prisma.rideRequest.findFirst({
  ...
  async findForDriver(
    ...
    const scored = openRequests.map((req) => {
      ...
      return {
        ...
        destDistKm,
        dayMatch,
        sortKey: dayScore + originScore * 0.7 + destScore * 0.3,
      };
    });
    ...
    return scored.map(({ req, originDistKm, destDistKm, dayMatch }) => ({
      ...
      originDistKm:
        originDistKm != null ? Math.round(originDistKm * 10) / 10 : null,

...
export default function InboxPage({ initialThreadId, onThreadClosed }: InboxPageProps = {}) {
  ...
  if (initialThreadId && !hasOpenedInitialThread.current && !openThreadId) {
    ...
    if (!thread) {
      return (
        <main className="bg-[#F7F7F9] min-h-screen px-4 pb-28 flex items-center justify-center">
          <div className="text-center">
            <div className="text-sm text-gray-500">A carregar conversa...</div>
          </div>
        </main>
      );
    }
  ...
  return (
    <main className="bg-[#F7F7F9] min-h-screen px-4 pb-28" role="main" aria-label="Caixa de entrada">
      {error && (

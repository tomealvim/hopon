...
export default function ThreadView({
  meId, thread, messages, onBack, onSystemAction, getUserName,
}: {
  meId: string;
  thread: Thread;
  messages: Message[];
  ...
  return (
    <div className="fixed inset-0 bg-white z-50 flex flex-col">
      <header className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 bg-white">
        <button className="text-2xl font-bold text-gray-900 hover:text-gray-700 transition" onClick={onBack} aria-label="Voltar">
          ←
        </button>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 bg-gray-50">
        {messages.map(m => (
          <div key={m.id} className={m.type === "text" ? (m.authorId === meId ? "flex justify-end mb-3" : "flex justify-start mb-3") : "mb-3"}>
            {m.type === "text" ? (
            <div className={m.authorId === meId ? "max-w-[75%] bg-primary text-white rounded-2xl px-4 py-2.5" : "max-w-[75%] bg-white text-gray-900 rounded-2xl px-4 py-2.5 shadow-sm border border-gray-100"}>
      ...
      <footer className="flex items-center gap-2 px-4 py-3 border-t border-gray-200 bg-white">
        <input className="flex-1 px-4 py-2.5 border border-gray-300 rounded-xl outline-none focus:border-primary focus:ring-2 focus:ring-primary/20" placeholder="Escrever mensagem" title="Escrever mensagem" />
        <button className="px-4 py-2.5 bg-primary text-white font-medium rounded-xl hover:bg-primary/90 transition">Enviar</button>
      </footer>
    </div>
  );
}
...
function SystemCardRenderer({
  ...
  if (ev.kind === "request_accepted") return <div className="text-center bg-green-50 text-green-800 text-xs py-2 px-3 rounded-lg">Pedido aceite ✅</div>;
  if (ev.kind === "request_declined") return <div className="text-center bg-red-50 text-red-800 text-xs py-2 px-3 rounded-lg">Pedido recusado</div>;
  if (ev.kind === "change_confirmed") return <div className="text-center bg-blue-50 text-blue-800 text-xs py-2 px-3 rounded-lg">Alteração confirmada</div>;
  if (ev.kind === "payment_paid") return <div className="text-center bg-green-50 text-green-800 text-xs py-2 px-3 rounded-lg">Pagamento confirmado 💳</div>;

  return <div className="text-center bg-gray-100 text-gray-600 text-xs py-2 px-3 rounded-lg">Atualização</div>;
}

...
export default function RideRequestSheet({ open, onClose, onSaved }: Props) {
  ...
  function reset() {
    setOriginLabel(""); setOriginLat(undefined); setOriginLng(undefined);
    setDestLabel(""); setDestLat(undefined); setDestLng(undefined);
    setDepartTime("");
    setDays(["segunda","terca","quarta","quinta","sexta"]);
    setNote("");
    setError(""); setLocError(""); setDone(false);
  }
  ...
  async function handleSubmit() {
    ...
    setError("");
    try {
      await apiRequest("/ride-requests", {
        method: "POST",
        body: JSON.stringify({
  ...
  return (
    <Sheet open={open} onClose={handleClose} title="Pedir boleia">
      {done ? (
        ...
        <div className="flex flex-col gap-5 pb-6">
          ...
          <Button className="w-full mt-2" disabled={!canSubmit} onClick={handleSubmit}>

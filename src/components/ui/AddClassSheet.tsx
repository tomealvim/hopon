...
export default function AddClassSheet({ open, onClose, onAdd, editBlock }: AddClassSheetProps) {
  ...
  return (
    <Sheet
      open={open}
      onClose={onClose}
      title={editBlock ? "Editar aula" : "Adicionar aula"}
      height="lg"
      footer={
    ...
    >
      <div className="grid gap-4">
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-700 mb-1">Disciplina (opcional)</label>
          <input
            className="w-full px-3 py-2.5 border border-gray-200 rounded-xl outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="Ex: PFI-T, Matemática..."
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-700 mb-1">Sala (opcional)</label>
          <input
            className="w-full px-3 py-2.5 border border-gray-200 rounded-xl outline-none focus:border-primary focus:ring-2 focus:ring-primary/20"
            value={room}
            onChange={(e) => setRoom(e.target.value)}
            placeholder="Ex: TA-A127, B203..."
            title="Sala ou local"
          />
        </div>
      </div>
